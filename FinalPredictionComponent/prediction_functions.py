...
def get_data(): 
    ...
    train_x, test_x, train_y, test_y = train_test_split(data_x, data_y, test_size=.3, random_state=42, shuffle=True)
...
def get_trained_model(train_x, train_y, test_x, test_y):
    NUM_EPOCHS = 200
    mlp = MLPClassifier(hidden_layer_sizes=(128, 96, 64, 32), verbose=True, max_iter=300, tol= 0.0000001, warm_start=WARM_START)
    # save_model(mlp, "only_adam")
    mlp.fit(train_x, train_y)
    # train it further using sgd
    mlp.solver='sgd'
    mlp.fit(train_x, train_y)
    return mlp
